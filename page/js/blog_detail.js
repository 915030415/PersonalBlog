let blogDetail = new Vue({
    ...
    created() {
        let searcheUrlParams = location.search.indexOf('?') > -1 ? location.search.split('?')[1].split('&') : '';// & 可能后面还拼接有其他参数
        //searcheUrlParams 获取url 后面 的文章id
        if(searcheUrlParams == ''){
            return
        }
        let bid = -10;//文章id
    ...
    computed:{
        ...
        sendComment() {
            return () =>{
                ...
                let bid = -10;//文章id   文章 id -1 为 关于 页面  为 -2 为 留言 页面
                for(let i =0; i < searcheUrlParams.length; i++){
                    if(searcheUrlParams[i].split('=')[0] == 'bid'){
                        try{
                            bid = searcheUrlParams[i].split('=')[1];
    ...
    computed:{
        ...
        queryCommets() {
            ...
            axios({
                ...
                blogComments.total = resp.data.data[0].count;// 不是箭头函数 this 不是指向 blogComments
            }).catch(function(resp) {
                console.log("请求错误");
            });
        },
          reply(){
              return (commentId,userName) =>{
                  let jup = document.getElementById('reply');
                 ...
                 // jup.setAttribute('href',"#send_comment"); // 锚点跳转

                  location.href = "#send_comment";// 跳到 这个id上 页面 跳到 评论的地方
              }
          }
    },
    created(){
